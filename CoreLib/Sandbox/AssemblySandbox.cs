...
namespace CoreLib
{
    ...
    public class AssemblySandbox : MarshalByRefObject, IAssemblySandbox
    {
        ...
        {
            _isBusy = true; //Set _isBusy = true when execution started
            ...
            object ConstructedClassObject = GetConstructedClassObject(MethodInfoObject, zippedConstructorParameters.Select(x => x.Type).ToArray(), zippedConstructorParameters.Select(x => x.Instance).ToArray());
            ...
            try
            {
                var bytes = Serialize(MethodInfoObject.Invoke(ConstructedClassObject, zippedMethodParameters.Select(x => x.Instance).ToArray()));
                _isBusy = false; // Mark _isBusy = false after the execution done
                return bytes;
            }
            catch (Exception e)
            {
                _isBusy = false; // Mark _isBusy = false after the execution error
                return new byte[] { };
            }
        }
        ...
        public void LoadAssembly(string name, byte[] bytes)
        {
            ...
            AssemblyMap[name.ToLower()] = assembly;
